...
def train(X, Y,location, training_epochs=300, batch_size=1000, LR= 0.001, n_hidden_1 = 400,n_hidden_2 = 160,n_hidden_3 = 160, traintestsplit = 0.01, LRdecay=0,Beta=.0001):
    ...
    loss = tf.reduce_mean(tf.square(pred - y))    # cost function: MSE
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(training_epochs):
            for i in range(total_batch):
                ...
                if LRdecay==1:
                    _, c = sess.run([optimizer, cost], feed_dict={x: X_train[idx, :], y: Y_train[idx, :],
                                                                  input_keep_prob: .8, hidden_keep_prob: .8,
            ...
            MSETime[epoch, 0]= c
            MSETime[epoch, 1]= sess.run(cost, feed_dict={x: X_val, y: Y_val, input_keep_prob: .8, hidden_keep_prob: .8, is_train: False})
...
def test(X, model_location, save_name, n_input, n_output, n_hidden_1 = 400, n_hidden_2 = 160, n_hidden_3 = 160, binary=0):
